...
import { OLYMPIA_HOST_PERF_TRACE, OLYMPIA_HOST_SLOW_LOG_MS, measure, nowMs, perfTime } from './perf'
...
export async function fetchHostData(matchCode: string): Promise<HostData | null> {
  ...
  const resolvedNameMap = await perfTime(`[perf][host] resolveDisplayNamesForUserIds n=${participantIds.length}`, () =>
  ...
  const { data: roundQuestions } = await (async () => {
    ...
    const cached = await perfTime(`[perf][host] cache.round_questions.byMatchId ${realMatchId}`, () =>
  ...
  const [{ data: winnerBuzz }, { data: recentBuzzes }, { data: recentAnswers }] = await Promise.all([
    currentQuestionId
      ? (() => {
          ...
          return perfTime(`[perf][host] supabase.buzzer_events.winner rq=${currentQuestionId}`, () =>
    ...
    currentQuestionId
      ? (() => {
          let query = olympia
            ...
            .eq('round_question_id', currentQuestionId)
          if (resetOccurredAt) query = query.gte('occurred_at', resetOccurredAt)
          return perfTime(`[perf][host] supabase.buzzer_events.recent rq=${currentQuestionId}`, () =>
