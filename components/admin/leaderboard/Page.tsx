    ...
    <div className="p-6" suppressHydrationWarning>
      <div suppressHydrationWarning>
        ...
        <p className="text-muted-foreground mt-1">
          Tổng điểm theo lớp (đã trừ vi phạm, cộng hoạt động). Cập nhật thời gian thực.
        </p>
      </div>

      <Card className="mt-3 shadow-sm">
        <CardHeader className="bg-muted/30 border-b">
          <CardTitle className="text-xl font-semibold">Xếp hạng theo lớp</CardTitle>
              ...
              <TableBody>
                {data.map((row, i) => {
                  ...
                  return (
                    <TableRow
                      key={row.class_id}
                      className={rank <= 3 ? "bg-accent/5 hover:bg-accent/10" : "hover:bg-muted/30"}
                    >
