...
export function useOlympiaGameState({ sessionId, initialData }: UseOlympiaGameStateArgs) {
  ...
  const [timer, setTimer] = useState<TimerSnapshot>(() => computeRemaining(initialData.session.timer_deadline))
    ...
    const subscribe = async () => {
      try {
        ...
        const channel = supabase
          ...
          .on('postgres_changes', { event: '*', schema: 'olympia', table: 'live_sessions', filter: `id=eq.${sessionId}` }, (payload) => {
            if (payload.new) {
          ...
          .on('postgres_changes', { event: '*', schema: 'olympia', table: 'round_questions' }, (payload) => {
            const nextQuestion = payload.new as RoundQuestionRow | null
            if (!nextQuestion) return
            if ((nextQuestion.match_id ?? matchId) !== matchId) return
          ...
          .on('postgres_changes', { event: '*', schema: 'olympia', table: 'match_scores' }, (payload) => {
          ...
          .on('postgres_changes', { event: 'INSERT', schema: 'olympia', table: 'buzzer_events' }, (payload) => {
