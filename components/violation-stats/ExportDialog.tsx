...
type RecordRow = {
  ...
  created_at: string
  score: number
  criteria: { id: string; name: string | null; group: string | null; subgroup: string | null } | null
  users?: { user_profiles?: { full_name?: string | null } | { full_name?: string | null }[]; user_name?: string | null }
}
...
export default function ExportReportDialog() {
  ...
  async function handleExport() {
    try {
      ...
      const { data: classes, error: cErr } = await supabase.from('classes').select('id,name').eq('grade_id', gradeId).order('name')
      if (cErr) throw new Error(cErr.message)
      const classIds = (classes || []).map((c: any) => c.id)
      if (classIds.length === 0) throw new Error('Không có lớp thuộc khối đã chọn')

      // fetch records in range for these classes
      let q = supabase
        .from('records')
        .select('class_id, created_at, score, criteria(id,name,group,subgroup), users:student_id(user_profiles(full_name),user_name)')
        .is('deleted_at', null)
      ...
      const sortedGroups = Array.from(groups.keys()).sort((a, b) => a.localeCompare(b, 'vi'))
      const groupToSubgroups = new Map<string, string[]>()
      for (const g of sortedGroups) {
        const subs = Array.from(groups.get(g)!.keys()).sort((a, b) => a.localeCompare(b, 'vi'))
        groupToSubgroups.set(g, subs)
      }

      // Aggregate per class
      const classMap = new Map<string, { name: string; byGroup: Map<string, { bySub: Map<string, { entries: string[]; count: number; deduction: number }>; totalDeduction: number }>; totalPoints: number }>()
      ...
      for (const r of recs) {
        ...
        const dateStr = new Date(r.created_at).toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })
        const entry = `${dateStr}: ${fullName}`

        if (!classMap.has(cls)) classMap.set(cls, { name: className, byGroup: new Map(), totalPoints: 0 })
        const classData = classMap.get(cls)!
        if (!classData.byGroup.has(g)) classData.byGroup.set(g, { bySub: new Map(), totalDeduction: 0 })
        const gData = classData.byGroup.get(g)!
        if (!gData.bySub.has(sg)) gData.bySub.set(sg, { entries: [], count: 0, deduction: 0 })
      ...
      for (const [cls, data] of classMap) {
        const totalDeductionAcross = Array.from(data.byGroup.values()).reduce((acc, g) => acc + (g.totalDeduction || 0), 0)
      ...
      const classEntries = Array.from(classMap.entries()).sort((a, b) => a[1].name.localeCompare(b[1].name, 'vi'))
      // First compute ranks
      const totals = classEntries.map(([id, d]) => d.totalPoints)
      const sortedTotals = Array.from(new Set(totals.slice().sort((a, b) => b - a)))
      ...
      const startStr = start ? new Date(start).toLocaleDateString('vi-VN') : 'all'
      const endStr = end ? new Date(end).toLocaleDateString('vi-VN') : 'all'
      a.href = url
      a.download = `bao_cao_khoi_${grades.find(g => g.id === gradeId)?.name || 'unknown'}_${startStr}_${endStr}.xlsx`
      ...
      <DialogContent className="sm:max-w-[560px]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1">
            <Label>Điểm cơ sở</Label>
            <Input type="number" value={baseScore} onChange={(e) => setBaseScore(Number(e.target.value || 0))} />
          </div>
          <div className="flex flex-col gap-1">
            <Label>Khối</Label>
        ...
        <DialogFooter>
          <Button onClick={handleExport} disabled={loading || !gradeId}>{loading ? 'Đang xuất...' : 'Xuất Excel'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
